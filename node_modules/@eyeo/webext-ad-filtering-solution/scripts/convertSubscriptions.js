...
async function convert(inDir, outDir,
  ...
  for (let inFile of files) {
    ...
    let ruleCount = await processFile(converter, lines, outFileAbs, spaces);
    ...
    for (let customSubscription of customSubscriptions) {
      if (customSubscription.id === subsId) {
        ...
        customSubscription.rule_count = ruleCount;
        break;
      }
    }
...
async function main() {
  const args = yargs(hideBin(process.argv))
    ...
    .option("safari", {
      boolean: true,
      description: "Convert to DNR rules supported by Safari (iOS)"
    })
    .parse();
  ...
  let safari = args.safari || false;
  if (safari) {
    console.warn("Converting for Safari");
  }
  await convert(inDir, outDir, generateReport,
                repDir, recommendedSubscriptions, spaces, safari);
}
